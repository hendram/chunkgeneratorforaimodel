    ...
    eventBus.on("sending again", async (signal) => {
      if (signal === true) {
        console.log(JSON.stringify({
          ...
          timestamp: new Date().toISOString(),
          logger: "kafka-producer",
          message: "Resending message to Puppeteer worker due to timeout",
          topic: "topuppeteerworker",
          payload_size: Buffer.byteLength(JSON.stringify(data), 'utf8'),
          payload_preview: JSON.stringify(data).slice(0, 100)
        }));
...
const puppeteerBuffer = {
  ...
  results: [],
   timeout: null             // to collect results
};
...
// Start/reset the 8-minute timer whenever a new job is received
  if (puppeteerBuffer.timeout) clearTimeout(puppeteerBuffer.timeout);
  puppeteerBuffer.timeout = setTimeout(async () => {
    console.log(" ^!^w Timeout reached, flushing incomplete jobs and resending...");
      eventBus.emit("sending again", true);
    // Cleanup buffer for next round
