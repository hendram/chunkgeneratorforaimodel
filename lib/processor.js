...
export async function vectorize(chunks) {  
  try {
    ...
    console.log(JSON.stringify({
      ...
      logger: "vectorizer-client",
      message: "Sending chunks to vectorizer",
      chunks_count: chunks.length,
      preview_chunk:  chunks.length > 0
    ? (chunks[0].text ? chunks[0].text.slice(0, 100) : JSON.stringify(chunks[0]).slice(0, 100))
    : null
    }));
    ...
    if (!resp.ok) {
      console.log(JSON.stringify({
        ...
        timestamp: new Date().toISOString(),
        logger: "vectorizer-client",
        message: `Vectorizer request failed`,
        status: resp.status,
        statusText: resp.statusText
      }));
     

       throw new Error(`Vectorizer error ${resp.status}`);
    }
 ...
 eventBus.emit("statusChanged", { returnValue: true });
  ...
  return resp.json();
    ...
    console.log(JSON.stringify({
      ...
      timestamp: new Date().toISOString(),
      logger: "vectorizer-client",
      message: "Vectorizer request exception",
      error: err.message
    }));
    throw err;
  }
...
function logResultVariable(result) {
  console.log(JSON.stringify({
    level: "INFO",
    timestamp: new Date().toISOString(),
    logger: "searchresult",
    message: "Search result received from backend",
    question: result.question,
    bestAnswer: result.bestAnswer,
    optionsCount: result.options ? result.options.length : 0,
...
function logBestVariable(best) {
  console.log(JSON.stringify({
    level: "INFO",
    timestamp: new Date().toISOString(),
    logger: "bestvectorized",
    message: "Best vectorized search result",
    question: best.question,
    bestAnswer: best.bestAnswer,
    url: best.metadata?.url || null
  }));
}
...
return("allBest", allBest);

}

// Utility function for structured logging
function logVectorizedCorporate(vectorized) {
  console.log(JSON.stringify({
    level: "INFO",
    timestamp: new Date().toISOString(),
    logger: "vectorizedcorporate",
    message: "Vectorized corporate update successful",
    status: vectorized.status,
    inserted_count: vectorized.inserted_count,
    internal_table: vectorized.internal_table
  }));
}

// This function only when creating table for _internal or _external
export async function handleResult(result) {
  try {

//This if part for creating _internal table
  if(result && result.resultscorporate  && result.resultscorporate.corporate){  
 const chunks = cleanAndChunk(result.resultscorporate.corporate || []);
    
      if (chunks && chunks.length > 0) {
        const vectorized = await vectorizecorporate(chunks);
        logVectorizedCorporate(vectorized);
...
//This part of if to create _external database
...
console.log(JSON.stringify({
  level: "INFO",
  timestamp: new Date().toISOString(),
  logger: "vectorized-client",
  message: "Vectorization completed successfully",
  status: vectorized.status,
  inserted_count: vectorized.inserted_count,
  external_table: vectorized.external_table,
  keyword_status: vectorized.keyword_status
}));          
