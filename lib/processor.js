...
`;
    ...
    console.log("Processing site with metadata:", site.metadata);
...
// --- Search ---
async function searchVectorDBSingle(question, option, metadata) {
  const payload = { 
    query: JSON.stringify({ question, option, metadata }), 
    top_k: 1 
  };
...
// Handle one QnA object and preserve metadata
export async function searchVectorDB(qna) {
  console.log("qnaonsearchvectordb", qna);

  const { question, options, metadata } = qna;
  const optionResults = [];

  // Loop over options
  for (const option of options) {
    const searchResult = await searchVectorDBSingle(question, option, metadata);
    optionResults.push({
      option,
      score: searchResult.top_results?.[0]?.score ?? Infinity
    });
  }
  ...
  // Return question + best answer + preserved metadata
  return {    
    question, 
